...
public class EncodersUtil {
    ...
    /**
     * The constant ORDER_DIRECTION.
     */
    public static final int ORDER_DIRECTION = 1;//顺序
    /**
     * The constant REVERSE_DIRECTION.
     */
     ...
     * @param maxLength  拼接的十进制数最大长度  例如:0x23  maxLength=2即可 ;两个字节:maxLength = 4
     * @param mDirection 拼接的方向
     * @return 得到的byte数组 byte [ ]
     */
     ...
     *
     * @param values     要拼接的十进制数
     * @param maxLength  拼接的十进制数最大长度,例如:1byte  maxLength = 1;
     * @param arg0       要拼接的头部字节数组
     * @param mDirection 尾部字节数组拼接方向
     * @return byte [ ]
     */
     ...
     *
     * @param arg0 :指定字节数组
     * @param data :出厂日期  例如: 20170208
     * @return byte [ ]
     */
    public static byte[] getBytes(byte[] arg0, String data) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < data.length(); i++) {
            sb.append("3");
            sb.append(data.charAt(i));
        }
        return arrayByteArray(arg0, hexString2Bytes(sb.toString()), ORDER_DIRECTION);
    }
